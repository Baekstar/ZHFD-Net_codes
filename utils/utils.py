...
def write_log_JC(file_name, JC): 
    fp = open(file_name, "a+")
    if isinstance(JC, torch.Tensor):
        min_val = JC.min().item()
        max_val = JC.max().item()
        mean_val = JC.mean().item()
        fp.write(f'HazefreeImage values min: {min_val:.6f}, max: {max_val:.6f}, mean: {mean_val:.6f}:\n')
    else:
        fp.write('azefreeImage values: %0.6f' % JC + ':\n')
    # fp.write('HazefreeImage values: %0.6f'%JC+':\n')
    fp.close() 