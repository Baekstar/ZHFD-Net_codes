...
def test(args):
    ...
    for i in range(len(input_img)):
        ...
        lap_pyramid = Lap_Pyramid_Conv().cuda(3)
        # highmerge = Highmerge().cuda(3)
        #### define model ####
        ...
        # Hazy = torch.from_numpy(Hazy).permute(2, 0, 1).unsqueeze(0).float().cuda(3)  # 转换为 (1, channels, height, width)
        ...
        # ground_truth = torch.from_numpy(ground_truth).permute(2, 0, 1).unsqueeze(0).float().cuda(3)  # 转换为 (1, channels, height, width)
        ...
        best_psnr = 0
        best_ssim = 0
        best_model_path='./best_model/SOTS-All.pth'
        #### train ####
        for k in tqdm(range(itr_no), desc="Loading..."):
            ...
            with autocast():
                ...
                hazy_depth=inference(Inputmage,model_path).cuda(3)
                rehaze_depth=inference(HazeProducemage,model_path).cuda(3)
                ...
                #### 上溢 下溢 ####
                otensor = torch.ones(HazefreeImage.shape).cuda(3)
                ztensor = torch.zeros(HazefreeImage.shape).cuda(3)
                ...
                if (k+1) % 200 == 1 or (k+1) % 200 == 0:
                    ...
                    J = (Inputmage - (1 - torch.from_numpy(refinet).cuda(3))*atm)/torch.from_numpy(refinet).cuda(3)
                        # HazefreeImage =HazefreeImage
                    # refinet = t_matting(Inputmage.detach().cpu().numpy(),high.detach().cpu().numpy(), trans[0].detach().cpu().numpy())
                    #J = (Inputmage - (1 - torch.from_numpy(trans).cuda(3))*atm)/torch.from_numpy(trans).cuda(3)
                    # J = (Inputmage - (1 - trans) * atm) / trans
                ...
                # SOTS
                    ...
                    torchvision.utils.save_image(hazy_depth, args.SavePath+'/'+input_img[i].split('_')[0]+'_depth_hazy.png')
                    torchvision.utils.save_image(rehaze_depth, args.SavePath+'/'+input_img[i].split('_')[0]+'_depth_rehaze.png')
        ...
        with torch.no_grad():
            ...
            _GT = (Input - (1 - torch.from_numpy(refine_t).cuda(3))*_atm)/torch.from_numpy(refine_t).cuda(3)

            _GT = torch.clamp(_GT, 0, 1)
        
        _out = apply_unsharp_mask(_out, amount=1.2, radius=1.5)  # 调整锐化参数
        ...
        write_log('./log/SOTS-All_matting.txt', input_img[i].split('_')[0], psnr_matting,ssim_matting)
        write_log('./log/SOTS-All_nor.txt', input_img[i].split('_')[0], psnr_normal, ssim_normal)
    ...
    write_log('./log/SOTS-All_matting.txt', 'average', avg_psnr, avg_ssim)
    write_log('./log/SOTS-All_nor.txt', 'average', avg_n_psnr, avg_n_ssim)
