...
from .highmerge import CALayer, PALayer
...
def blockUNet1(in_c, out_c, name, transposed=False, bn=False, relu=True, dropout=False):
    ...
    if not transposed:
        block.add_module('%s_conv' % name, nn.Conv2d(in_c, out_c, 4, 1, 1, bias=False))
    else:
        block.add_module('%s_tconv' % name, nn.ConvTranspose2d(in_c, out_c, 4, 1, 1, bias=False))
    if bn:
...
def blockUNet(in_c, out_c, name, transposed=False, bn=False, relu=True, dropout=False):
    ...
    if relu:
        ...
        block.add_module('%s_leakyrelu' % name, nn.LeakyReLU(0.2, inplace=True))
    if not transposed:
        block.add_module('%s_conv' % name, nn.Conv2d(in_c, out_c, 3, 2, 1, bias=False))
    else:
...
class atp_cal(nn.Module):
    ...
    def forward(self, x):
        ...
        print(f"out6 shape: {out6.shape}")
        out7 = self.layer7(out6)  # [1,64,2,2]            256
        print(f"out7 shape: {out7.shape}")

        out7 = self.ca2(out7)
        print(f"out7 shape: {out7.shape}")

        dout7 = self.dlayer7(out7)  # [1,64,4,4]       256
        print(f"dout7 shape: {dout7.shape}")
        # 调整 out6 的尺寸使其匹配 dout7 的尺寸
        # out6 = F.interpolate(out6, size=dout7.shape[2:])
        dout7_out6 = dout7 + out6  # [1,128,4,4]
        dout7_out6 = self.ca3(dout7_out6)

        dout6 = self.dlayer6(dout7_out6)  # [1,64,8,8]       256
        # 调整 out6 的尺寸使其匹配 dout7 的尺寸
        # out5 = F.interpolate(out5, size=dout6.shape[2:])
        dout6_out5 = dout6 + out5  # [1,128,8,8]
        dout5 = self.dlayer5(dout6_out5)  # [1,64,16,16]     256
        # 调整 out6 的尺寸使其匹配 dout7 的尺寸
        # out4 = F.interpolate(out4, size=dout5.shape[2:])
        dout5_out4 = dout5 + out4  # [1,128,16,16]
        dout4 = self.dlayer4(dout5_out4)  # [1,32,32,32]      256
        # 调整 out6 的尺寸使其匹配 dout7 的尺寸
        # out3 = F.interpolate(out3, size=dout4.shape[2:])
...
class Parameter(nn.Module):
    def __init__(self):
        ...
        self.batch1 = nn.InstanceNorm2d(20)
