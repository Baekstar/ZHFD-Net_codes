...
class estimation(nn.Module):
    def __init__(self):
        ...
        self.OutConv = nn.Conv2d(64, 1, 3, padding = 1, stride=1, bias=False, padding_mode='reflect')
        self.up = nn.Upsample(scale_factor=4, mode='bilinear', align_corners=True)
        # 增加MixStructureBlock用于透射图分支增强
        # self.mix_block2 = MixStructureBlock(1)  
...
class MainModel(nn.Module):
    ...
    def forward(self, hazy, high,flag):
        ...
        # # 去雾后的图像进行增强
        # scale=0.05
        # enhance=self.enhancement_model(haze)
        # haze=(haze+enhance*scale)/(1+scale)


        # atp = F.interpolate(atp, size=hazy.shape[2:])
        # tran = F.interpolate(tran, size=hazy.shape[2:])

        out = x_up4
        # return zz, atp, haze
        return tran, atp, haze
